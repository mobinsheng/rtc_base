...
extern "C" {
#endif
RTC_NORETURN void rtc_FatalMsg(const char* file, int line, const char* msg);
#ifdef __cplusplus
}  // extern "C"
#endif
...
#define RTC_CHECK(condition)                                             \
  do {                                                                   \
    if (!(condition)) {                                                  \
      rtc_FatalMsg(__FILE__, __LINE__, "CHECK failed: " #condition); \
    }                                                                    \
  } while (0)
...
#define RTC_DCHECK(condition)                                             \
  do {                                                                    \
    if (RTC_DCHECK_IS_ON && !(condition)) {                               \
      rtc_FatalMsg(__FILE__, __LINE__, "DCHECK failed: " #condition); \
    }                                                                     \
  } while (0)
