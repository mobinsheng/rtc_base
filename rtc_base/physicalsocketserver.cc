...
int PhysicalSocket::Recv(void* buffer, size_t length, int64_t* timestamp) {
  int received =
      (int)(::recv(s_, static_cast<char*>(buffer), static_cast<int>(length), 0));
...
int PhysicalSocket::RecvFrom(void* buffer,
  ...
  sockaddr* addr = reinterpret_cast<sockaddr*>(&addr_storage);
  int received = (int)(::recvfrom(s_, static_cast<char*>(buffer),
...
int PhysicalSocket::DoSend(SOCKET socket, const char* buf, int len, int flags) {
  return (int)(::send(socket, buf, len, flags));
}
...
int PhysicalSocket::DoSendTo(SOCKET socket,
                             ...
                             socklen_t addrlen) {
  return int(::sendto(socket, buf, len, flags, dest_addr, addrlen));
}
